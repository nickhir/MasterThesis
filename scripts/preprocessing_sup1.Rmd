...
### Introduction
Visualize the data before and after different filtering steps. See Methods section for details
...
### Show number of cells after filtering
```{r}
...
ggsave(filename = "/g/huber/users/hirschmueller/DECODE/MasterThesis/plots/preprocessing.png",
       dpi=500, width =8, height = 12.5)

```
 

```{r, echo=T, fig.width=8, fig.height=12.5, fig.align="center", dpi=300}
combined_plot
```

```{r,eval=F}
rmarkdown::render("/g/huber/users/hirschmueller/DECODE/MasterThesis/scripts/preprocessing_sup1.Rmd", 
