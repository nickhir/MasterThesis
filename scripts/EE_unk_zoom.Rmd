---
title: "EE unkown population zoom"
...
### Introduction
EE subclustering and zoom on the unkown EE population. 
...
notch_zoom <- DimPlot(notch_subset,
              group.by = "EE_subtype",
        label = T, label.box = T, 
        repel = T,pt.size = 0.4, label.size = 3.7,) +
...
ctrl_zoom <- DimPlot(ctrl_subset,
              group.by = "EE_subtype",
        label = T, label.box = T, 
        repel = T,pt.size = 0.6, label.size = 3.7)+
...
## Combine the plots
```{r}
combined_plot <-wrap_plots(notch_umap,plot_spacer(),notch_zoom, plot_spacer(),pca_ee_subtypes,
                           ctrl_umap,plot_spacer(),ctrl_zoom, plot_spacer(),pca_ee_subtypes_ctrl,
           nrow = 2,ncol = 5) +
  plot_layout(widths = c(1,0.07,1,0.07,1,1,0.07,1,0.07,1))+
  plot_annotation(tag_levels = "A") & 
  theme(plot.tag = element_text(size = 18, face="bold"))

ragg::agg_png("/g/huber/users/hirschmueller/DECODE/MasterThesis/plots/ee_zoom.png", width = 16, height = 10,
              res=600, units = "cm", scaling = 0.4)
combined_plot
dev.off()
