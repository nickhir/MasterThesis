---
title: "Figure 5 Differential expression analysis"
...
seurat@meta.data$tmp_celltype <-case_when(
  seurat$celltype_manual %in% c("EB","ISC") ~ "ISC/EB/EEP",
  T ~ seurat$celltype_manual
)


# load in the EE subtype annotation
...
mdata_ee <- rbind(notch_ee, ctrl_ee)

seurat@meta.data <- seurat@meta.data %>% rownames_to_column("Barcode_unique") %>% 
  left_join(., mdata_ee) %>% 
  mutate(EE_subtype = ifelse(is.na(EE_subtype),"x",EE_subtype))%>% # odd things happen with ifelse if there are NAs
  mutate(tmp_celltype=ifelse(EE_subtype=="EEP","ISC/EB/EEP",tmp_celltype))
...
sce <- sce[,sce$tmp_celltype =="ISC/EB/EEP"]
...
dds_subset <- DESeqDataSetFromMatrix(countData = counts(reduced_sce),
                              colData = colData(reduced_sce),
                              design= ~ perturbation)
...
volc_plot <- res_deseq %>% 
  ...
  volcano_plot(.,
               ...
               annotate_by=c("sff", "E(spl)m6-BFM","meso18E", "N","E(spl)mgamma-HLH"),
               annotation_size=5.9,
               min_pval_cutoff  = 10e-15)+
...
kegg_gsea<-gsea_res@result %>% 
  ...
  mutate(ID_short = c("Ribosome", "Mismatch Repair", "Base Excision Repair", "Homologous\nRecombination", "Nucleotide\nExcision Repair", "Cell Cycle", "Proteasome", "DNA Replication")) %>% 
...
```{r}
ragg::agg_png("/g/huber/users/hirschmueller/DECODE/MasterThesis/plots/fig5_differential_expression_analysis.png", width = 16, height = 6, res=500, 
              units = "cm", scaling = 0.4)
wrap_plots(volc_plot, gsea_kegg_res, boxplots, nrow=1) + 
  plot_layout(widths = c(0.7,0.6,0.9)) &
...
![](/g/huber/users/hirschmueller/DECODE/MasterThesis/plots/fig5_differential_expression_analysis.png)
